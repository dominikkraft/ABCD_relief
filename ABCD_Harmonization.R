...
# Combat and Relief harmonization and saves raw/combat/relief data files for .py scripts

rm(list = ls())


# set datapath !
DATAPATH = "~/ABCD_release5/data/abcd-data-release-5.0/core/" 
...
# choose only scanners from SIEMENS (prisma,fit) and GE (Discovery)
demo3 <- demo3[demo3$mri_info_manufacturersmn %in% list("Prisma", "Prisma_fit", "DISCOVERY MR750"),]
...
harmonization_function <- function(data_frame, output_prefix, size) {
...
for (si in sample_sizes) {
  ...
  harmonization_function(fa_sampled, "fa", si)
  harmonization_function(md_sampled, "md", si)
}
